# Git Installation Assignment Submission

## Student Information

**Name:** [Varun Krishna.S]

**Student ID:** [2025it0613]

**Date:** [10/02/2026]

**Operating System:** [Windows]

---

## Installation Details

### Git Version Installed

Paste the output of `git --version` here:

```
[2.53.0.windows.2]
```

### Username (whoami output)

Paste the output of `whoami` here:

```
[varun krishna .s]
```

---

## Screenshot

**Screenshot File Path:** `screenshots/git_installation_varun_krishna.png`

**Screenshot Description:** 
["Git Bash window showing whoami command with output 'varun krishna .s'"]

---

## Installation Experience

### Installation Method Used

["Downloaded installer from git-scm.com", "Used Homebrew", etc.]

### Did you encounter any issues during installation?

[No]

---

## Verification

- [YES] Git is successfully installed on my machine
- [YES] I can run `git --version` command successfully
- [YES] I can run `whoami` command successfully
- [YES] I have taken a screenshot showing the `whoami` command execution
- [YES] I have added my screenshot to the `screenshots/` folder
- [YES] I have filled out all required information in this file
- [YES] I have committed and pushed my changes to the repository

---

## Additional Comments

[Nice platform for learning.]

---

**Submission Date:** [10/02/2026]

**Repository URL:** [https://github.com/ait-svce/getting-started-with-git-VARUNKRISHNA-08-1.git]
